# GL后端MSAA WBOIT Resolve问题详解分析与修复方案

## 背景

MSAA Resolve是将多采样帧缓冲区降采样为常规帧缓冲区的过程。Vulkan通过子流程的解析附件在渲染流程结束时隐式完成（multisampled framebuffer）降采样为常规帧缓冲区的过程。Vulkan通过子流程（subpass）的解析附件（resolve attachment）在渲染流程结束时隐式完成，解析是渲染流程结构的一部分，而非独立操作；OpenGL ES则需要显式调用`glBlitFramebuffer`或使用`GL_EXT_multisampled_render_to_texture2`扩展。WBOIT管线对此尤为敏感——它同时使用两个颜色附件：累加缓冲区（accumulation，`r16g16b16a16_sfloat`）和揭示度缓冲区（revealage，`r16_unorm`），任一附件解析失败均会导致透明物体渲染错误。
...
GL后端WBOIT管线中，`r16_unorm`格式的揭示度缓冲区目标纹理在MSAA解析前后内容不一致。

## 问题根本原因分析

### 1. drawBuffers索引错误（影响空图像场景高优先级）

**位置：** `node_context_pool_manager_gles.cpp:44530`、`node_context_pool_manager_gles.cpp:530`

GenerateSubPassFBO`第440行

当前代码使用递增计数器构造`drawBuffers`索引，当附件中存在空图像（null image）时，计数器与循环索引实际附件位置产生偏移，导致FBO颜色附件位置被压缩。详见 `Potential Issues/FBO颜色附件位置映射错误详解.md`。
...
glBlitFramebuffer(0, 0, width, height, 0, 0, width, height, GL_COLOR_BUFFER_BITmask, GL_NEAREST);
}
```

**对WBOIT的影响**：WBOIT有2个解析附件（`resolveAttachmentCount > 1`），走`ResolveMSAAMultiColor`路径。该路径创建独立临时FBO，逐附件绑定到`GL_COLOR_ATTACHMENT0`后blit，**不依赖**预构建FBO的布局（不受drawBuffers bug影响）。resolve路径本身正确代码第1661行注释已标注："NOTE: ARM recommends NOT to use glBlitFramebuffer here"。ARM Mali驱动在MSAA解析场景下对`glBlitFramebuffer`存在已知兼容性问题。
...
| **多颜色附件** | `VkSubpassDescription`中`colorAttachments`与`resolveAttachments`一一对应的并行数组，驱动同时处理 | `ResolveMSAAMultiColor`逐附件创建临时FBO对，逐个blit所有映射 | `GenerateResolveFBO`中逐个迭代，但单次`glBlitFramebuffer`只处理一个读取缓冲区 |
...
**修改1（第44530行 — GenerateSubPassResolveFBO）：**
```cpp
// 当前代码（BUG）:
drawBuffers[idx] = GL_COLOR_ATTACHMENT0 + colorAttachmentCount;

// 修复后:
drawBuffers[idx] = GL_COLOR_ATTACHMENT0 + idx;rp.resolveAttachmentCount;
...
**修改2（第53440行 — GenerateResolveSubPassFBO）：**
```cpp
// 当前代码（BUG）:
drawBuffers[idx] = GL_COLOR_ATTACHMENT0 + rp.resolveAttachmentCount;
...
> 注意：`colorAttachmentCount`和`rp.resolveAttachmentCount`修复后仍保留递增逻辑，因为它们在其他上下文中有实际用途，只是不应再决定attachment位置。详见 `Potential Issues/FBO颜色附件位置映射错误详解.md`colorAttachmentCount;
BindToFbo(...);
...
|--------|--
|--------|----------|----------|
| 1 | 修复1（drawBuffers修正） | 高 — 修复空图像时附件映射错位 | 存在空附件的渲染流程错误的附件映射 |
| 2 | 修复2（逐附件解析） | 关键 — 使多附件解析正常工作 |
| 23 | 修复2（格式验证3（验证增强） | 中等 — 早期捕获格式错误 | r16_unorm格式回退场景 |
...
| 文件 | 行号 | 修改内容 |
|------|------|----------|
| `node_context_pool_manager_gles.cpp` | 44530, 53440 | 修复drawBuffers索引 |
...
- Vulkan规范：子流程解析附件要求
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1Nzg4OTk5OTddfQ==
-->