...
- [2. 渲染数据存储](#2-渲染数据存储)
  - [2.1 IRenderDataStoreDefaultCamera（OIT类型管理接口）1. [类与接口设计](#21-irenderdatastoredefaultcameraoit类型管理接口)
...
#### 1.1.1 RenderConfigurationComponent（OIT配置入口）
...
public:
    ...
    float vpcfRadius;               // 阴影类型（PCF/VSM/VARIABLE_PCF）
...
> **注意**：以下字节偏移量为近似值，实际偏移取决于宏展开、对齐和填充。**内存布局**：
...
public:
    ...
    BASE_NS::Math::Vec4 scissor;               // 视口[x, y, width, height]
...
## ## 1.1.3 MaterialComponent（OIT材质标志）
...
#### 1.2.1 IRenderDataStoreDefaultCamera（OIT类型管理接口）接口
...
## ## 1.2.2 RenderDataStoreDefaultCamera（实现）类

**类定义**：

```cpp
// 路径: lume/Lume_3D/src/render/datastore/render_data_store_default_camera.hcpp
...
public:
    ...
    const BASE_NS::string_view GetTypeName() const override { return TYPE_NAME; }
...
6. 名称映射**：`unordered_map` 提供名称快速查找
3. **OIT检测**：AddCamera时自动检测OIT_BIT标志
4. **默认值**：`oitType_` 默认为 `WBOIT`，最优性能
5. **线程安全**：接口方法**：`GetTypeName()` 返回 `TYPE_NAME`，`GetName()` 返回实例名 `name_`声明 "Not internally synchronized"，调用者需保证同步

---

#### 1.2.3 RenderCamera（OIT标志位与ShaderFlags）结构体
...
struct RenderCamera {
    ...
    **ShaderFlags shaderFlags;
    ...
    uint32_t sceneId;**                // 着色器标志位（包含OIT shader flags）
    ...
    TargetUsage  // 自定义渲染节点图文件

    // 颜色和深度目标定制
    TargetCustomization colorTargetCustomization[8];
    TargetUsageCustomization depthTargetCustomization;
};
```
...
```
RenderSystem (render_system.cpp) 内联OIT相机检测逻辑::ProcessOitCameras()
...
#### 1.3.1 RenderNodeDefaultMaterialRenderSlotLloit（LLOIT节点）
...
// Staging缓冲（用于CPU到GPU数据传输）
    ...
    struct CurrentScene {
        ...
        BASE_NS::vector<uint32_t> mvCameraIndices;  // OIT shader flags
    };
    ...
    const uint32_t width, const uint32_t height)
{
    ...
    uint32_t linkedListH（每像素一个uint32_t）
    ...
    string_view linkedListHconst GpuBufferDesc headBufferName =Desc = {
    ...
    lliotGpuResources_.LinkedListHeadBuffer_ =
            gpuResourceMgr.Create(l"liotGpuResources_.LinkedL_listH_headB_buffer_", linkedListHheadBufferDesc);
    }

    // 2. 创建LinkedListNodeBuffer
    uint32_t linkedList（所有节点）
    ...
    lliotGpuResources_.LinkedListBuffer_ =
            gpuResourceMgr.Create(l"liotGpuResources_.LinkedL_listB_buffer_", linkedListnodeBufferDesc);
    }
 ...
 （计数器）
    const GpuBufferDesc counterBufferDesc = {
       ...
       lliotGpuResources_.LinkedListCounterBuffer_ =
            gpuResourceMgr.Create(l"liotGpuResources_.LinkedL_listC_counterB_buffer_", linkedListCcounterBufferDesc);
    }
    ...
    if (lliotGpuResources_.LinkedListCounterBuffer_) {
        ...
        CopyDataToBuffer(counterDataCopyDataToBuffer(initialHeadData, lliotGpuResources_.LinkedListHeadBuffer_);
...
| LinkedListNodeBuffer | width × height × MAX_FRAGMENT_COUNT16 × sizeof(LinkedListNode) | ~531132.7 MB | 所有片段节点（16/像素） |
...
IMaterialComponentManager <|-[#2196F3]- MaterialComponent : 组件创建
...
LloitGpuResources *--[#9C27B0] LinkedListNode : 包含节点数组

' === 使用关系 ===
RenderNodeDefaultMaterialRenderSlotLloit ..[#673AB7]> RenderDataStoreDefaultCamera : 读取OIT类型
RenderNodeDefaultMaterialRenderSlotLloit ..[#673AB7]> RenderCamera : 读取ShaderFlags
...
## 4.1 链表节点结构（DefaultOitLinkedListNodeStruct）# 2.1 GPU数据结构详细设计

#### 2.1.1 LinkedListNode结构体
...
struct DefaultOitsrc/render/node/render_node_default_material_render_slot_lloit.h
...
// 16字节对齐，匹配CPU端总大小: sizeof(LinkedListNode) = 16 bytes
```
...
| **1080p总内存** | 318 MB | color(16) + depth(4) + next(4) = 24 bytes | color(8) + depth(4) + next(4) = 16 bytes | **33%** |
| 1080p总内存（16片段/像素） | 1920×1080×16×24 = 318 MB | 1920×1080×16×16 = **141 MB** | 颜色压缩 | 56% | <0.1% |
...
## 4.3 SSBO布局定义（3d_dm_oit_layout## 2.1.3 DefaultOitLinkedListNodeStruct（GLSL定义）
...
### Pass 1：材质着色器（set=3，仅Vulkan）struct DefaultOitLinkedListNodeStruct {
...
void InplaceLinkedListOit(in vec4 color, in vec3 fragCoord, in     // 当前节点索引
...
void InplaceLinkedListOit(vec4 color, vec3 fragCoord, uint imageWidth) {
    ...
    // 2. 溢出检测：超出最大节点数则丢弃检查缓冲溢出
    if (currNodeIdx < maxNodeIdx) {
        // 3. 计算像素索引
        ivec2 ifragCoord = ivec2(fragCoord.xy);
        uint pixelIndex = ifragCoord.y * imageWidth + ifragCoord.x;

        // 4. 原子交换更新链表头（新片段成为链表头链表头（获取前一个头索引）
        uint prevHead = atomicExchange(LinkedListHead[pixelIndex], currNodeIdx);
...
if ((CORE_CAMERA_FLAGS & CORE_CAMERA_OIT_PPLL_BIT) == CORE_CAMERA_OIT_PPLL_BIT) {
    ...
    float fDepths[OIT_MAX_FRAGMENT_COUNT];   // 最多16个16压缩
        nodes[currNodeIdx].depth = fragCoord.z;               // 深度值
...
#define INVALID_NODE_IDX 0xFFFFFFFF         // 链表终点标记（无效索引）
...
#define OIT_MAX_VISIBILITY_NODE_COUNT 8  // 最大可见性节点数（+1存储背景）
...
```cpp    nodes[currNodeIdx].color = PackVec4Half2x16(color);
    nodes[currNodeIdx].depth = fragCoord.z;
    nodes[currNodeIdx].next = prevHead;
} else {
    // 缓冲溢出：丢弃片段
...
#define CORE_DM_CONSTANT_ID_CAMERA_FLAGS 4  // Camera shader flags传递
...
#define INSERT_SORT 1  // 排序算法选择：1=插入排序，0=冒泡排序
...
**文档结束**
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMDg4NTgyNF19
-->