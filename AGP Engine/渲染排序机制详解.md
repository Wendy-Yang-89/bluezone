...
```
sortKey = (renderSortLayer << 56) | (renderSortLayerOrder << 48) | depthBits
```

sortKey是64位整数，按位存储三层排序信息，用于渲染队列排序。
...
│ Layer排序 (最高优先级)                            │
├──────────────────────────────────────────────────┤
│ sortKey |= (renderSortLayer << 56)               │
│ 例如: Layer=32                                   │
│   └─► sortKey = 0x2000000000000000               │
└──────────────────────────────────────────────────┘
                      │
                      ▼
┌──────────────────────────────────────────────────┐
│ Order排序 (次优先级)                              │
├──────────────────────────────────────────────────┤
│ sortKey |= (renderSortLayerOrder << 48)          │
│ 例如: Order=100                                  │
│   └─► sortKey += 0x0064000000000000              │
└──────────────────────────────────────────────────┘
                      │
                      ▼
┌──────────────────────────────────────────────────┐
│ Depth排序 (根据sortType)                          │
├──────────────────────────────────────────────────┤
│ BY_MATERIAL: depthBits = materialSortId          │
│ FRONT_TO_BACK: depthBits = depth (近→远优先)      │
│ BACK_TO_FRONT: depthBits = ~depth (远→近优先)     │
│                                                  │
│ sortKey |= depthBits                             │
└──────────────────────────────────────────────────┘
                      │
