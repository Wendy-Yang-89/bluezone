...
| `hash` | uint64_t | 着色器状态数据的哈希值，用于快速查找 |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `pso` | RenderHandle | 创建的PSO句柄 |
| `set3` | bool | 是否需要自定义绑定Set3 |
...
## 返回值
- `PsoAndInfo`: 包含PSO句柄和是否需要自定义绑定Set3标志的结构体
...
### 关键函数调用
- `shaderMgr.GetShaderHandle()` / `GetGraphicsStateHandleByShaderHandle()` / `GetGraphicsStateHandle()` - Shader和GraphicsState查找
- `GetEvaluatedPipelineLayout()` - 管线布局评估
- `GetNewGraphicsState()` - 修改图形状态（反转绕序/自定义IA时）
- `GetShaderSpecView()` - 着色器特化常量生成
- `psoMgr.GetGraphicsPsoHandle()` - PSO创建或缓存获取
