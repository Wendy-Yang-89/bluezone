...
Scene ──────────────────────────────────────────────────── 顶层容器，多场景隔离
 ...
 │    │  RenderMeshData
 │    ├── mesh       ──► 引用 MeshComponent Entity（不修改材质）
...
| RenderMeshComponent | `RenderMeshData` | `mesh : uint64_t`<br>`layerMask : uint64_t`<br>`sceneId : uint64_t` | 网格实例 |
...
| 筛选机制 | 代码位置 | 筛选规则 | 说明 |
|---------|---------|---------|------|
| **layerMask** | `camera_component.h:253-254` | `RenderMeshData.layerMask & Camera.layerMask != 0` → 可见 | uint64_t 位运算，LayerFlagBits 定义各层 |
...
```cpp
// api/3d/ecs/components/camera_component.h:253-254
DEFINE_BITFIELD_PROPERTY(uint64_t, layerMask, "Layer mask",
    PropertyFlags::IS_BITFIELD, VALUE(LayerConstants::ALL_LAYER_MASK), LayerFlagBits)
...
// api/3d/ecs/components/layer_defines.h:98
struct LayerConstants {
    DEFAULT_LAYER_MASK = CORE_LAYER_FLAG_BIT_00,  // 0x1
    ALL_LAYER_MASK     = CORE_LAYER_FLAG_BIT_ALL,  // 所有层
    NONE_LAYER_MASK    = 0,                          // 无层
};
```
...
| `api/3d/ecs/components/camera_component.h` | `CameraComponent`（253-254行：layerMask） |
...
| `api/3d/ecs/components/layer_defines.h` | `LayerFlagBits : uint64_t`（28行），`LayerConstants`（98行） |
