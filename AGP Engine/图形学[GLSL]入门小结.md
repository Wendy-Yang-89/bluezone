...
- GLSL version 3.30版本前，不支持opaque类型数组；
- GLSL version 3.30版本开始支持Sampler类型（该版本仅有的Opaque类型）数组，但sampler数组索引必须是常量表达式；
- GLSL 4.00及以上版本，支持opaque数组，但是opaque数组索引必须是**dynamically uniform**表达式。
...
**Dynamically Uniform的含义**：如果一个表达式在**同一渲染批次的所有invocation**上产生相同的值，则称为"dynamically uniform"。包括：
- 常量表达式（`const`变量、字面值）
- `uniform`变量
- 同一批次中所有invocation共享的值（如compute shader的`gl_WorkGroupID`）
...
| layout内存对齐相关 | `packed`，`shared`，`std140`，`std340`，`row_major`，`colum_major` |
